...
function vita_workers (conf, cpuset)
   ...
   worker.public_router = configure_public_router_with_nic(conf)
...
function capsule_workers (conf, cpuset)
   ...
   for id, _ in pairs(conf.route) do
      ...
      attributes["encapsulate_"..id] = {scheduling={cpu=cpuset[3+cpu]}}
      cpu = cpu + 1
      workers["decapsulate_"..id] = configure_dsp(conf, id)
      attributes["decapsulate_"..id] = {scheduling={cpu=cpuset[3+cpu]}}
   end
   return workers, attributes
end
