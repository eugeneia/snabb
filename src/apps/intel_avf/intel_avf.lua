...
function Intel_avf:mbox_sr_caps()
   ...
   print("rss_key_size", tt.rss_key_size)
   self.rss_lut_size = tt.rss_lut_size
   print("rss_lut_size", tt.rss_lut_size)
end
...
function Intel_avf:mbox_s_rss()
   -- pg83
   -- Attempt to forcefull disable the RSS feature.
   -- (Set every related knob to zero.)
   local tt = self:mbox_send_buf(virtchnl_rss_key_ptr_t)
   tt.vsi = self.vsi_id
   tt.key_len = self.rss_key_len
   -- Can I set choose the keylen? What’s the semantics here. FUCK. Check DPDK
   -- source code maybe. First try advertising RSS caps. Maybe this solves
   -- itself with some fiddling.
   self:mbox_sr('VIRTCHNL_OP_CONFIG_RSS_KEY',
                ffi.sizeof(virtchnl_rss_key_t)-1 self.rss_key_size - 1)
   local tt = self:mbox_send_buf(virtchnl_rss_lut_ptr_t)
   tt.vsi = self.vsi_id
   self:mbox_sr('VIRTCHNL_OP_CONFIG_RSS_LUT',
                ffi.sizeof(virtchnl_rss_lut_t)-1 + self.rss_lut_size)
