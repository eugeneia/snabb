...
CRCERRS  0x04000 -             RC CRC Error - R/clr
...
GPRC     0x04074 -             RC Good Packets Receive Count - R/clr
BPRC     0x04078 -             RC Broadcast Packets Received - R/clr
MPRC     0x0407C -             RC Multicast Packets Received - R/clr
GPTC     0x04080 -             RC Good Packets Transmitted - R/clr
...
function Intel:new (arg)
   ...
   -- For now we expose per-device statistics from the “master” app.
   if self.master then
      self.shmpath = "/counters/"..pci.qualified(self.pciaddress).."/"
...
local Intel1g_provided_counters = {
...
function Intel1g:init ()
   ...
   -- Setup counters
   for _, name in ipairs(Intel1g_provided_counters) do
      self.counters[name] = counter.open(self.shmpath..name)
   end
   counter.set(self.counters.type, 0x1000) -- Hardware interface
   counter.set(self.counters.dtime, C.get_unix_time())
   counter.set(self.counters.speed, 1000000) -- 1 Gbits
   counter.set(self.counters.status, 2) -- down
end

function Intel1g:sync_stats ()
   if not self.master then return end
   local counters = self.counters
   local r = self.r
   local status = r.STATUS()
   counter.set(counters.status, lib.bitset(status, 1) and 1 or 2)
   counter.set(counters.speed, (({10000,100000,1000000,1000000})[1+bit.band(bit.rshift(status, 6),3)]))
   -- 7.9 Statistics Counters
   counter.set(counters.rxbytes, r.GORCH() *2^32 +r.GORCL())
   counter.set(counters.rxpackets, r.GPRC())
   local mprc, bprc = r.MPRC(), r.BPRC()
   counter.set(counters.rxmcast, mprc + bprc)
   counter.set(counters.rxbcast, bprc)
   counter.set(counters.rxdrop, r.MPC() + r.RNBC())
   counter.set(counters.rxerrors,
               r.CRCERRS() + r.RLEC() + r.RXERRC() + r.ALGNERRC())
   counter.set(counters.txbytes, r.GOTCH() *2^32 +r.GOTCL())
   counter.set(counters.txpackets, r.GPTC())
   local mptc, bptc = r.MPTC(), r.BPTC()
   counter.set(counters.txmcast, mptc + bptc)
   counter.set(counters.txbcast, bptc)
   counter.set(counters.txdrop, r.ECOL())
   counter.set(counters.txerrors, r.LATECOL())
end
...
function Intel1g:stop ()
   ...
   for name, _ in pairs(self.counters) do
      counter.delete(self.shmpath..name)
   end
   shm.unlink(self.shmpath)
end
...
local Intel82599_provided_counters = {
   'type', 'dtime', 'mtu', 'speed', 'status', 'promisc',
   'rxbytes', 'rxpackets', 'rxmcast', 'rxbcast', 'rxdrop', 'rxerrors',
...
function Intel82599:init ()
   ...
   self.r.MAXFRS(lshift(self.mtu, 16))
   ...
   if self.rxq then
      self:rss_enable()
      -- Count RX stats in Receive Queue Statistics Register 0.
      self.r.RQSMR[math.floor(self.rxq/4)]:set(lshift(0,8*(self.rxq%4)))
   end

   -- Setup counters
   for _, name in ipairs(Intel82599_provided_counters) do
      self.counters[name] = counter.open(self.shmpath..name)
   end
   counter.set(self.counters.type, 0x1000) -- Hardware interface
   counter.set(self.counters.dtime, C.get_unix_time())
   counter.set(self.counters.mtu, self.mtu)
   counter.set(self.counters.speed, 10000000) -- 10 Gbits
   counter.set(self.counters.status, 2) -- down
end
...
function Intel82599:sync_stats ()
   ...
   local counters = self.counters
   local r = self.r
   counter.set(counters.rxbytes,   r.GORC64())
   counter.set(counters.rxpackets, r.GPRC())
   local mprc, bprc =              r.MPRC(), r.BPRC()
   counter.set(counters.rxmcast,   mprc + bprc)
   counter.set(counters.rxbcast,   bprc)
   ...
   counter.set(counters.rxdrop,    r.QPRDC[0]())
   counter.set(counters.rxerrors,  r.CRCERRS() + r.ILLERRC() + r.ERRBC() +
                                   r.RUC() + r.RFC() + r.ROC() + r.RJC())
   counter.set(counters.txbytes,   r.GOTC64())
   counter.set(counters.txpackets, r.GPTC())
   local mptc, bptc =              r.MPTC(), r.BPTC()
   counter.set(counters.txmcast,   mptc + bptc)
   counter.set(counters.txbcast,   bptc)
